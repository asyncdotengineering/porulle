...
Tenancy fails closed. `resolveOrgId` no longer falls back to a boot-default organization when there is no actor, so a request that cannot prove which organization it belongs to is refused rather than served the first one. Set `auth.strictOrgResolution: false` (or `STRICT_ORG_RESOLUTION=false`) to restore the previous permissive behaviour during a migration.
