...
const API_KEY = process.env.PORULLE_API_KEY!;
...
async function api(method: string, path: string, body?: unknown, opts?: { noAuth?: boolean }) {
  ...
  if (!opts?.noAuth) headers["x-api-key"] = API_KEY;
...
const BASE = "http://localhost:4000";
const HEADERS = { "x-api-key": process.env.PORULLE_API_KEY!, "Content-Type": "application/json" };
