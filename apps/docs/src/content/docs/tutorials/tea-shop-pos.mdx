...
All commands send `x-api-key: $PORULLE_API_KEY`. Mint it with `porulle api-key create --scope admin` against a scope you declared in `auth.apiKeyScopes`; there is no built-in development key.
