...
export default defineConfig({
  ...
  auth: {
    ...
    // Required. Without this (or `storeResolver`) the server refuses to start.
    defaultOrganizationId: "org_acme",
    apiKeys: { enabled: true },
    // Keys are minted against a named scope — there is no built-in dev key.
    // Mint one with:
    //   porulle api-key create --scope admin --save-env --env-var PORULLE_API_KEY
    apiKeyScopes: {
      admin: {
        prefix: "acme_adm_",
    ...
    roles: {
      ...
      staff: {
        permissions: [
          ...
          // Reading draft / not-yet-visible entities is a distinct scope, not
          // something catalog:update implies. Without it a staff member sees
          // no unpublished product.
