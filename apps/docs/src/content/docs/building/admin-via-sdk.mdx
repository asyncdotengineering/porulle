...
- An admin API key. Create one with the CLI:
  ...
  The CLI reads `./commerce.config.ts` (override with `--config`) and boots it in
  process — it does not talk to a remote server, so there is no `--server` flag.
  `--scope` must name a scope declared in `auth.apiKeyScopes`; the permissions
  come from that scope, not from the command line.
