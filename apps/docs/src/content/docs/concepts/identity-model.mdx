...
A single-store deployment sets `auth.defaultOrganizationId`; a multi-store one sets `auth.storeResolver`. **One of them is required** — since 0.14.0 `createServer` refuses to construct with neither, rather than booting and returning 503 on the first actor-less request.

`strictOrgResolution` defaults to **on**. A request that carries no actor and cannot be resolved to an organization is rejected with HTTP 503 instead of falling back to a default org. The old permissive fallback served one merchant's data to unauthenticated callers on every unguarded read path, which is why it is no longer the default. Opt out with `auth.strictOrgResolution: false` or `STRICT_ORG_RESOLUTION=false` only while migrating.
