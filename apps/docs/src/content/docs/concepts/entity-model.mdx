...
**The catalog service** uses `fields` to validate each entity's `customFields` values on create and update. Declaring `weight` as a number rejects strings; an undeclared field name is rejected as unknown. Values land as typed rows in `sellable_custom_fields` — each carrying provenance (`source`, `status`, `confidence`, locale) — not in the metadata blob. Field definitions are not config-only: org-scoped runtime definitions layer over the code config without a deploy, and `select` fields enforce their option vocabulary. See [Catalog Custom Fields](/building/custom-fields/). The `metadata` JSONB column stays free-form for anything undeclared.
...
The cost of JSONB is that the database does not enforce the schema of the `metadata` blob. Declared custom fields avoid most of this — their values live in typed columns on `sellable_custom_fields` — but validation against the field definitions still happens at the application layer, not as a PostgreSQL `CHECK` constraint. Direct SQL writes bypass it.
