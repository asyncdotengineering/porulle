...
| `include` | `string` | Comma-separated: `attributes`, `variants`, `optionTypes`, `categories`, `brands`, `media`, `inventory`, `pricing`, `customFields` |

`include=customFields` returns the entity's approved custom-field rows (with provenance columns) under `data.customFields`.
...
```json
{
  ...
  "status": "draft",
  "metadata": {},
  "customFields": { "material": "linen" }
}
```

`customFields` is validated against the entity type's field definitions. On `PATCH`, it is a patch: an explicit `null` clears a field, omitted keys are untouched. See [Catalog Custom Fields](/building/custom-fields/).

### Custom-field review

| Method | Path | Description |
|--------|------|-------------|
| `POST` | `/catalog/entities/:id/custom-fields/:fieldName/approve` | Approve the newest proposal — displaces the live value atomically (`catalog:update`) |
| `POST` | `/catalog/entities/:id/custom-fields/:fieldName/reject` | Reject the newest proposal (`catalog:update`) |

Both accept a `locale` query parameter (default `en`). The org-scoped proposal queue lives at [`GET /api/admin/custom-field-proposals`](#admin-catalog).

### Field ownership

| Method | Path | Description |
|--------|------|-------------|
| `GET` | `/catalog/entities/:id/field-ownership` | List ownership rows, optional `?storeId` (`catalog:read`) |
| `PUT` | `/catalog/entities/:id/field-ownership` | Set an owner: `{ fieldPath, owner: "platform" \| "store" \| "shared", storeId?, variantId? }` (`catalog:update`) |

See [Catalog Field Ownership](/building/catalog-ownership/) for path grammar and precedence.
...
| `status` | `string` | Filter by status |
| `attr.<name>` | `string`, repeatable | Filter by a custom-field attribute (equality only) |
| `page` | `number` | Page number (default `1`) |
| `limit` | `number` | Results per page (default `20`, max `100`) |
| `facets` | `string` | Comma-separated facet fields — built-ins (`type`, `category`, `brand`, `status`) and attribute names |

Attribute filters use repeatable `attr.<name>=<value>` parameters: repeated values for one name are ORed, distinct names are ANDed, matching is equality only. Attribute names must match `^[A-Za-z0-9_-]+$` — anything else returns `422`. Only fields whose definition sets `filterable: true` are indexed, using approved `en`-locale values; facet counts are keyed by attribute name.

Response includes `data` (hits array) and `meta` with `total`, `page`, `limit`, `facets`.

Without a configured search adapter, the in-memory fallback requires at least a query or one filter — a facets-only request returns an empty result rather than loading the whole catalog. Both shipped adapters (`@porulle/adapter-pg-search`, `@porulle/adapter-meilisearch`) implement attribute filtering and facets.
...
Every asset carries an `origin` (`merchant` | `generated` | `imported`, default `merchant`), an optional `confidence`, and an optional `derivedFromAssetId` link to its source asset. REST uploads are always `merchant`; the service-level `media.upload()` accepts an `origin` (channel imports stamp `imported`).

**`DELETE /media/:id`** — merchant-origin assets are write-guarded: deleting one requires `media:write`.
...
Attachments are unique per level: the same asset may attach once at entity level and once per variant, but a duplicate at the same level returns `409`.
...
| Method | Path | Description |
|--------|------|-------------|
| `POST` | `/api/payments/webhook/:provider` | Payment provider webhook receiver |

Accepts raw webhook payloads from the payment adapter named by `:provider`, which must match that adapter's `providerId` — an unregistered provider returns `404` and records nothing. The delivery is verified by that adapter and recorded against it, so two gateways can be registered at once without either seeing the other's events.

For the bundled Stripe adapter (`/api/payments/webhook/stripe`), a `payment_intent.succeeded` event carrying `metadata.orderId` transitions that order to `confirmed` automatically. That handling is Stripe's own; another gateway's events are verified and recorded but do not transition an order, because their event vocabulary is not core's to interpret.
...
Base path: `/api/admin`. All routes require `catalog:update`.

| Method | Path | Description |
|--------|------|-------------|
| `GET` | `/admin/custom-field-proposals` | Org-scoped queue of proposed custom-field values, optional `?entityType`, paginated |
| `GET` | `/admin/entity-field-definitions` | List runtime field definitions, optional `?entityType` |
| `POST` | `/admin/entity-field-definitions` | Create a runtime field definition |
| `PATCH` | `/admin/entity-field-definitions/:id` | Update a definition |
| `POST` | `/admin/entity-field-definitions/:id/archive` | Archive a definition (archive-only lifecycle — no delete) |

Proposal items carry the full custom-field row plus `entitySlug` and `entityType`. See [Catalog Custom Fields](/building/custom-fields/).
...
| `GET /admin/custom-field-proposals`, `/admin/entity-field-definitions` group | `catalog:update` |
| Custom-field `approve`/`reject` | `catalog:update` (+ `catalog:sync` for store-sourced entities) |
| `GET /catalog/entities/:id/field-ownership` | `catalog:read` |
| `PUT /catalog/entities/:id/field-ownership` | `catalog:update` (+ `catalog:sync` for store-sourced entities) |
| `DELETE /media/:id` (merchant-origin assets) | `media:write` |
