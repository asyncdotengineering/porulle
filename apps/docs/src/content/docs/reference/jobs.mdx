...
The kernel auto-registers only `webhooks/deliver` and `jobs/reap-stale`; plugins contribute their tasks through their manifests. The channel connector registers `channel/backfill-catalog`, `channel/sync-inventory`, `channel/push-order`, `channel/reconcile`, `channel/reconcile-sweep`, and `channel/reap-exports` — all keyed per store via `concurrency.key`. Connecting a store starts no import; the host starts one from its own operator route. `channel/backfill-catalog` processes one page per run and re-enqueues itself until the backfill completes (see [Catalog backfill](/building/channel-connectors/#catalog-backfill)).
