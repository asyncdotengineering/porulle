...
| `routes` | `(app: Hono, kernel: ConfigRouteKernel, auth?: AuthInstance) => void` | — | Register custom Hono routes. `kernel.database` carries both the raw handle and a tenant-scoped `kernel.database.scoped`; the third argument is the Better Auth instance. See [Custom tables](/extending/custom-tables/). |
...
| `auth` | `number` | `10` | Max requests per minute for credential authentication endpoints |
| `session` | `number` | `120` | Max session reads per minute; session reads have a separate per-IP bucket |
| `checkout` | `number` | `5` | Max requests per minute for checkout creation |
| `signInPerEmail` | `number` | `10` | Max sign-in attempts per email per 15 minutes |
...
interface AuthConfig {
  ...
  apiKeyScopes?: Record<string, ApiKeyScope>;
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `defaultOrganizationId` | `string` | — | Organization ID for single-store deployments. Required unless `storeResolver` is set — `createServer` refuses to construct without one of them. See [Identity and Store Resolution](/concepts/identity-model/). |
| `strictOrgResolution` | `boolean` | **`true`** | Fail closed. A request that carries no actor and cannot be resolved to an organization is rejected with HTTP 503 rather than falling back to a default org. Set `false` (or `STRICT_ORG_RESOLUTION=false`) only to restore the pre-0.13 permissive behaviour during a migration. |
| `apiKeyScopes` | `Record<string, ApiKeyScope>` | — | Named API-key scopes, each with a `prefix`, `permissions`, and optional `description`. `porulle api-key create --scope <name>` mints against these. |
...
|-------|------|-------------|
| `fields` | `EntityFieldDefinition[]` | Custom field definitions. The catalog service validates each entity's `customFields` values against these on create/update; values are stored in `sellable_custom_fields`. |
...
| `filterable` | `boolean` | Index this field's approved `en`-locale values as search attributes. Default `false`. |
| `localized` | `boolean` | Mark the field as per-locale. Default `false`. |
...
| `options` | `string[]` | Allowed values (for `"select"` fields). Enforced exact-match after trimming whitespace — an out-of-vocabulary value is rejected with `422` naming the allowed set. A `select` without `options` is unconstrained. |
| `sortOrder` | `number` | Display/merge order. Default `0`. |

Code-declared fields are the base layer. Runtime rows in `entity_field_definitions` (managed via `/api/admin/entity-field-definitions`) layer over them per organization — see [Catalog Custom Fields](/building/custom-fields/).
