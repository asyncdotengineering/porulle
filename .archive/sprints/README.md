...
This directory is the operating system of the Foundation Repair (`unified-commerce-engine`) build. Four things live here:
...
## The mission across all 6 sprints

**Bring the foundation to "production-honest" before anyone says the word "framework" out loud.**

Source: `FRAMEWORK-WIKI.md` (Phase 1 architectural blueprint) + `FRAMEWORK-WIKI-PHASE-2.md` (non-obvious findings: live bugs, multi-tenancy hazards, dead infrastructure, DX papercuts, strategic position).

| Sprint | Phase | One-line goal |
|--------|-------|---------------|
| 0 | Foundation hygiene + critical correctness | Fix silent data corruption + add LICENSE + make starter boot |
| 1 | Multi-tenancy hardening | Close every cross-tenant data-leak vector |
| 2 | Live bug fixes + reaper | Fix the six live bugs that ship today |
| 3 | Documentation honesty + onboarding | Implement or delete every documented-but-unimplemented feature |
| 4 | Service container modernization | Replace property-bag with typed `defineModule` |
| 5 | Typed hooks + framework extraction | Typed hook map + internal `packages/framework` extraction |

After Sprint 5, the foundation is publication-honest. **External publication of `@aeronyx/framework` is still a backlog item** (B-09) — it requires a second internal application + a production customer + a co-maintainer or sponsor before it's defensible. See `FRAMEWORK-WIKI-PHASE-2.md` §7 for the strategic case.
...
| Why is this project being run? | [`/FRAMEWORK-WIKI.md`](../FRAMEWORK-WIKI.md) + [`/FRAMEWORK-WIKI-PHASE-2.md`](../FRAMEWORK-WIKI-PHASE-2.md) |
| What did sprint N do? | `sprint-{N}/WARMDOWN.md` |
| What does sprint N+1 need to know? | `sprint-{N}/HANDOFF.md` |
| Why was decision X made? | `sprint-{N}/review-sprint-r1.md` + `gate-sprint.md` + `review-sprint-r2.md` (or wiki commit body for amendments) |
